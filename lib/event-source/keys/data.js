...
exports.parse = function dataParseGenerate (key) {
  ...
  if (parse.length > 1) {
    ...
    for (max = parse.length; i < max; i++) {
      var splitKey = retVal.splitKey || (retVal.splitKey = [])
      if (parse[i].length > 0) {
        splitKey.push(parse[i])
      }
    }
    if (retVal.splitKey && retVal.splitKey.length > 0) {
      retVal.key = retVal.splitKey.join(KEYSPLIT)
    } else {
      delete retVal.splitKey
    }
  }
  return retVal
}
