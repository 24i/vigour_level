...
var getMonthViaOrdinalDay = function (days, isLeap) {
  var month
  var ordinalDay
  isLeap = (isLeap) ? 1 : 0
...
var getDayOfWeek = function getFirstDayWeek (year, week, dayOfWeek) {
  week = week || 1
...
var notations = {
  periods: {
    ...
    'M': function periodMonth (amount, add, date, dates) {
      // calculateAmountWithAlreadySetAmounts(type, amount, setAmount)
    ...
    'D': function periodDay (amount, add, date, dates) {
      ...
      console.log(date.getUTCFullYear() * 365.25, date.getUTCMonth() * 30.4375, date.getUTCDate(), setAmount, add)
      console.log(Math.floor((date.getUTCFullYear() * 365.25 + date.getUTCMonth() * 30.4375 + date.getUTCDate()) / setAmount), Math.floor((date.getUTCFullYear() * 365.25 + date.getUTCMonth() * 30.4375 + date.getUTCDate()) / setAmount) + add, (Math.floor((date.getUTCFullYear() * 365.25 + date.getUTCMonth() * 30.4375 + date.getUTCDate()) / setAmount) + add) * setAmount)
      console.log('startDay', startDay)
      console.log('year', Math.floor(startDay / 365.25))
      console.log('month', startDay % 365.25, Math.floor((startDay % 365.25) / 30.4375))
      console.log('day', ((startDay % 365.25) % 30.4375))
    ...
    if (period.charAt(i).toUpperCase() === 'P') {
      ...
      for (var d = 0, dMax = notations[curType].all.length; d < dMax; d++) {
        if (period.charAt(i) === notations[curType].all.charAt(d)) {
          console.log(period, curType, amount, period.charAt(i))
