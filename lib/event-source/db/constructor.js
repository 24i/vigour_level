...
eventdb._setDb = function EventDbSet (type) {
  ...
  if (this.switchInProgres) {
    ...
    db[operation](keys.events.generate(stamp, key), value, function dataOperation (e) {
      if (!e) {
        ...
        console.log('put Data', key, value, values.data.generate(value, stamp))
        self._getDb('data')[operation](key, values.data.generate(value, stamp), callback)
        if (doSwitch) {
          self.switchDb()
        }
      } else {
...
var closeDb = function (list, dbStack, i, endCallback) {
  if (i < list.length) {
    dbStack[list[i]].close(function () {
      closeDb(list, dbStack, (i + 1), endCallback)
    })
  } else {
    if (endCallback) {
      return endCallback()
...
eventdb.close = function EventDbClose (cb) {
  cb = cb || function () { return true }
  var dbs = Object.keys(this.dbStack)
  closeDb(dbs, this.dbStack, 0, cb)
}
