...
const utcTimestamp = require('../utils/utc-timestamp').current
...
var setDb = function setDb (name, period, add) {
  if (period) {
    ...
    return {
      ts: timeStamp,
      name: name + '-' + timeStamp,
      db: levelup(name + '-' + timeStamp, {cacheSize: 16 * 1024 * 1024, db: levelDB})
    }
  }
  return {
    name: name,
...
eventdb._setDb = function (type) {
  if (this.dbTypes[type]) {
    ...
    if (!this.dbStack[dbKey]) {
      this.dbStack[dbKey] = levelup(this.name + '-' + dbKey, {cacheSize: 16 * 1024 * 1024, db: levelDB})
...
eventdb._execOperation = function (operation, stamp, key, value, callback) {
  ...
  if (this.switchInProgres) {
    ...
    db[operation](keys.generate(stamp, key), value, function dataOperation (e) {
      if (!e) {
        if (!db._stamp) {
          db._stamp = stamp
        }
        self._getDb('data')[operation](key, value, callback)
        if (doSwitch) {
          self.switchDb()
        }
      } else {
